import React from 'react';
import Link from 'next/link';

const OurMagicSauce = () => {
    return (
        <section className="pt-100 pb-70">

            <div className="container">
                <div className="row">
                    <div className="col-lg-9">
                        <div className="section-title">
                            <h2>Our magic sauce: what makes Tonomy ID so special</h2>
                            <p>Our zero-knowledge architecture is what makes Tonomy ID so special. In Tonomy ID, all user data and authentication secrets are store exclusively in the person's phone and never on our servers unlike most digital identity solutions. This mitigates significant security and privacy issues by ensuring that even if our servers are breached, no user data is compromised. This prevents a large amount of common problems found in most systems. When fully utilized, Tonomy ID practically removes the following issues from your system:</p>
                        </div>

                        <div className="row">
                            <div className="col-lg-12">
                                <div className="about-img">
                                    <img src="/images/OurMagicSauce1.PNG" alt="Image" />
                                </div>
                            </div>
                        </div>

                        <div className="row pt-20">
                            <div className="col-lg-4 col-sm-6">
                                <div className="single-offer">
                                    <i className="flaticon-chip"></i>
                                    <h3>
                                        <a>Server-side security breaches</a>

                                    </h3>

                                </div>
                            </div>

                            <div className="col-lg-4 col-sm-6">
                                <div className="single-offer">
                                    <i className="flaticon-vr"></i>
                                    <h3>

                                        <a>GDPR and data privacy compliance"</a>

                                    </h3>

                                </div>
                            </div>

                            <div className="col-lg-4 col-sm-6">
                                <div className="single-offer">
                                    <i className="flaticon-blockchain"></i>
                                    <h3>

                                        <a>Phishing attacks</a>

                                    </h3>

                                </div>
                            </div>

                            <div className="col-lg-4 col-sm-6">
                                <div className="single-offer">
                                    <i className="flaticon-target"></i>
                                    <h3>

                                        <a>Server-side privacy leaks</a>

                                    </h3>

                                </div>
                            </div>

                            <div className="col-lg-4 col-sm-6">
                                <div className="single-offer">
                                    <i className="flaticon-choice"></i>
                                    <h3>

                                        <a>Data sharing and consent control</a>

                                    </h3>

                                </div>
                            </div>

                            <div className="col-lg-4 col-sm-6">
                                <div className="single-offer">
                                    <i className="flaticon-deep-learning"></i>
                                    <h3>

                                        <a>Authentication security compromises</a>

                                    </h3>

                                </div>
                            </div>

                            <div className="col-lg-4 col-sm-6">
                                <div className="single-offer">
                                    <i className="flaticon-deep-learning"></i>
                                    <h3>

                                        <a>Data processing visibility</a>

                                    </h3>

                                </div>
                            </div>

                            <div className="col-lg-4 col-sm-6">
                                <div className="single-offer">
                                    <i className="flaticon-deep-learning"></i>
                                    <h3>

                                        <a>Compromised website security</a>

                                    </h3>

                                </div>
                            </div>


                        </div>
                        <div className="row">
                            <div className="col-lg-12">
                                <div className="about-img">
                                    <img src="/images/OurMagicSauce2.PNG" alt="Image" />
                                </div>
                            </div>
                        </div>
 
                    </div>

                </div>


            </div>

        </section>
    );
}

export default OurMagicSauce;